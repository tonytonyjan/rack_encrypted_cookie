...
describe Rack::Session::EncryptedCookie do
  ...
  after do
    Rack::Session::EncryptedCookie.class_eval { remove_method :warn }
  end

  describe 'Base64' do
    it 'uses base64 to encode' do
      coder = Rack::Session::EncryptedCookie::Base64.new
      str   = 'fuuuuu'
    ...
    describe 'Marshal' do
      it 'marshals and base64 encodes' do
        coder = Rack::Session::EncryptedCookie::Base64::Marshal.new
        str   = 'fuuuuu'
    ...
    describe 'JSON' do
      it 'marshals and base64 encodes' do
        coder = Rack::Session::EncryptedCookie::Base64::JSON.new
        obj   = %w(fuuuuu)
    ...
    describe 'ZipJSON' do
      it 'jsons, deflates, and base64 encodes' do
        coder = Rack::Session::EncryptedCookie::Base64::ZipJSON.new
        obj   = %w(fuuuuu)
        json = JSON.dump(obj)
  ...
  it 'warns if no secret is given' do
    Rack::Session::EncryptedCookie.new(incrementor)
    @warnings.first.must_match(/no secret/i)
    @warnings.clear
    Rack::Session::EncryptedCookie.new(incrementor, secret: 'abc')
    @warnings.must_be :empty?
  end

  it "doesn't warn if coder is configured to handle encoding" do
    Rack::Session::EncryptedCookie.new(
      incrementor,
      coder: Object.new,
  ...
  it 'barks on too big cookies' do
    ...
    end.must_raise Rack::MockRequest::FatalWarning
  end
  ...
  it "allows more than one '--' in the cookie when calculating digests" do
    @counter = 0
    app = lambda do |env|
      env['rack.session']['message'] ||= String.new
